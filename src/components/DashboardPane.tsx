...
export function DashboardPane() {
  ...
  const toggleCardSize = (id: number) => {
    setCardSizes(prev => ({ ...prev, [id]: prev[id] === 'full' ? 'half' : 'full' }));
  };
  ...
  const renderVisual = (query: any) => {
    ...
    if (loading[query.id]) return <div className="flex items-center justify-center h-full text-slate-400 text-sm">Loading...</div>;
    if (!data || data.length === 0) return <div className="flex items-center justify-center h-full text-slate-400 text-sm">No data</div>;
    ...
    switch (query.visual_type) {
      ...
      case 'pie':
        return (
          <ResponsiveContainer width="100%" height="100%">
            <RechartsPieChart>
              <Pie data={data} dataKey={measureKeys[0]} nameKey={dimKey} cx="50%" cy="50%" outerRadius={80} innerRadius={40} paddingAngle={2}>
  ...
  return (
    <div className="p-8 h-full overflow-y-auto bg-slate-50">
      <div className="mb-8">
        ...
        <p className="text-slate-500 mt-1">Your saved queries and visualizations. Use the resize button to expand a card.</p>
      </div>
      ...
      {savedQueries.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {savedQueries.map(query => {
            const isFullWidth = cardSizes[query.id] === 'full';
            return (
              <div
                key={query.id}
                className={`bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col transition-all duration-300 ${isFullWidth ? 'md:col-span-2 xl:col-span-3 h-96' : 'h-72'}`}
              >
                <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                  <h3 className="font-semibold text-slate-800 truncate pr-4">{query.name}</h3>
                  <div className="flex items-center gap-2 shrink-0">
                    <button
                      onClick={() => toggleCardSize(query.id)}
                      className="text-slate-400 hover:text-blue-500 transition-colors"
                      title={isFullWidth ? 'Reduce' : 'Expand'}
                    >
                    ...
                    <button onClick={() => executeQuery(query)} className="text-slate-400 hover:text-emerald-500 transition-colors">
                      <RefreshCw size={15} className={loading[query.id] ? "animate-spin" : ""} />
                    </button>
